...
export default async function ContratoLocacao({
  params, searchParams,
}: { params: { id: string }; searchParams: { ok?: string; pago?: string; repasse?: string; encerrado?: string; multa?: string } }) {
  ...
  return (
    <>
      ...
      <div className="phead">
        ...
        <Link className="btn-outline" href={`/painel/locacao/${c.id}/vistoria`} style={{ fontSize: ".85rem" }}>📋 Vistorias</Link>
      </div>
      ...
      {searchParams.encerrado && (
        <p className="ok" style={{ marginBottom: "1rem" }}>
          ✔ Contrato encerrado — parcelas futuras canceladas.
          {searchParams.multa && <> Multa rescisória proporcional calculada: <strong>{brl(Number(searchParams.multa))}</strong> — lançada como Prevista no Financeiro (cobre do locatário e marque como recebida lá).</>}
        </p>
      )}
